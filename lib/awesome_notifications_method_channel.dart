...
class MethodChannelAwesomeNotifications extends AwesomeNotificationsPlatform {
  ...
  @override
  Future<List<NotificationPermission>> checkPermissionList({String? channelKey,
  ...
  @override
  Future<bool> requestPermissionToSendNotifications({String? channelKey,
    List<NotificationPermission> permissions = const [
      NotificationPermission.Alert,
      NotificationPermission.Sound,
      NotificationPermission.Badge,
  ...
  @override
  Future<bool> setListeners({required ActionHandler onActionReceivedMethod,
    ...
    print('setListeners called in method channel');
    print('onActionReceivedMethod $onActionReceivedMethod');
    print('actionHandler $actionHandler');

    actionHandler.add(onActionReceivedMethod);
    print('actionHandler 1 $actionHandler');
  ...
  Future<dynamic> handleMethod(MethodCall call) async {
    ...
    switch (call.method) {
      ...
      case EVENT_DEFAULT_ACTION:
        var received = ReceivedAction().fromMap(arguments);
        print('received action in method channel 1 ${actionHandler.length}');
        print('received action in method channel call  $call');

        int i = 0;
        for (ActionHandler item in (actionHandler ?? [])) {
          await item(received);
          print('received action in method channel loop  ${received.hashCode}');

          i += 1;
          print(
              'received action matched in method channel received key omar  ${received}');
          print(
              'received action matched in method channel received key abdo  ${received
                  .buttonKeyPressed}');
          print(
              'received action matched in method channel received key ${call
                  .arguments["buttonKeyPressed"]}');
          if (call.arguments[NOTIFICATION_BUTTON_KEY_PRESSED] ==
              received.buttonKeyPressed) {
            print('item number is called   $i');
            print('received action in method channel call 3  $call');
            // actionHandler.remove(item);

          }
        }
        print('received action in method channel 2 ${actionHandler.length}');
